...
row_ind.extend([eq_num, eq_num, eq_num])
# data.extend([1/h[0], -1/h[0], -q/(2 * math.pi * rw)])
data.extend([1, -1, -h[0]*rw/(rw+h[0]/2)])
# data.extend([1, -1, -h[0]*rw/(rw*(1/rw)**1/N_min)])
eq_num += 1
# col_ind.extend([p_ind(0), p_ind(1)])
# row_ind.extend([eq_num, eq_num])
# data.extend([-(rw+h[0]/2)/h[0], (rw+h[0]/2)/h[0]])
# rhs[eq_num] = q/(2*math.pi)
# eq_num += 1
# col_ind.extend([p_ind(1), p_ind(2), u_ind(0)])
# row_ind.extend([eq_num, eq_num, eq_num])
# data.extend([(rw+h[0]/2+rw+h[0]+h[1]/2)/(2*h[1]), -(rw+h[0]/2+rw+h[0]+h[1]/2)/(2*h[1]), rw*(rw+rw+h[0]/2)/(2*(rw+h[0]/2))])
# rhs[eq_num] = q/(2*math.pi)
# eq_num += 1
...
plt.grid(which='minor', linestyle=':')
plt.ylim([-0.1, 1.1])
name_p_file = f'Функция давления (задание 2.1.1) N={xs}.png'
...
plt.grid(which='minor', linestyle=':')
plt.grid(which='minor', linestyle=':')
name_u_file = f'Функция скорости фильтрации (задание 2.1.1) N={xs}.png'
...
with open(f'results (задание 2.1.1).txt', 'w', encoding='utf-8') as file:

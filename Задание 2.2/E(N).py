...
for N_min in range(10, 10001, 50):
    print(N_min)
    # Кол-во зон и их границы
    with open('zone_info(E(N)).txt', 'r') as file:
...
plt.figure(1)
plt.plot(N_arr, E_arr, 'r')
plt.title('Погрешность p от числа узлов')
plt.minorticks_on()
plt.grid()
plt.xlabel('N')
plt.ylabel('E')
...
plt.grid(which='minor', linestyle=':')
name_p_file = f'Погрешность p от числа узлов.png'
plt.savefig(name_p_file, dpi=300)

plt.figure(2)
plt.loglog(N_arr, E_arr, 'r')
plt.title('Погрешность p от числа узлов (логарифм)')
plt.minorticks_on()
plt.grid()
plt.xlabel('N')
plt.ylabel('E')
...
plt.grid(which='minor', linestyle=':')
plt.grid(which='minor', linestyle=':')
name_p_file = f'Погрешность p от числа узлов (логарифм).png'
plt.savefig(name_p_file, dpi=300)